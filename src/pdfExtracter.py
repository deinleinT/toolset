...
class MyThread(threading.Thread):
    ...
    def run(self):
        ...
        for s in self.__pdfList: 
            ...
            print("Processing Standard " + str(s) + " in Thread " + str(threading.current_thread().getName()) + ", Thread progress: " + str(progress) + "% ...\n")
            try:
                pdfFile = PyPDF2.PdfFileReader(s)
                pageCount = pdfFile.getNumPages()
            except Exception as e:
                print (str(e))
                allScopes[s] = "Error with Standard " + str(s) + " " + str(e)
                continue
            pagesFound = []
            startFound = False
            try:
...
try:
    ...
    start = "scope"
    stop = "references"
    textFileName = "scopesOfStandards"
    direct = "./Standards"
