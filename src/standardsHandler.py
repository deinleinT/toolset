...
def __convert_to_pdf(doc, newName):
    ...
    try:
        ...
        # worddoc.SaveAs(newNameTwo.replace(".pdf", ".docx"), FileFormat=12)
        flag = True
...
def extractZipFilesAndConvertToPDF(nameZip, destination):
    ...
    # todo extract the standard number
    converted = False
...
def download(srcurl, dstfilepath):
    ...
    if not os.path.exists("./Standards/" + dstfilepath):
        with open("./Standards/" + dstfilepath, "wb") as code:
            code.write(r.content)
            result = True
    
    if os.path.exists("./Standards/" + dstfilepath):
        result = True
...
specs = dict()
noUpdate = set()
datumDifference = "checkStandardsBetween" + initial[1] + "And" + initial[2] + time.strftime("%d%m%Y%H%M") + "_"
...
print("\n*** Using " + str(numberThreads) + " Threads for comparing the versions of all standards in the Excel-Sheets... ***\n")
##################################################
#
...
# Thread downloading the zips only if flag
